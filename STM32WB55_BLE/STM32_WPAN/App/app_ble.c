...
/* Funcion que desconecta el dispositivo de cualquier conexion BLE activa
 *
 */
void APP_BLE_Disconnect(void){
	hci_disconnect(BleApplicationContext.BleApplicationContext_legacy.connectionHandle, 0x05);
}

/* Funcion que pone la radio RF en modo stand by
 *
 */
void APP_BLE_Enter_Standby_Mode(void){
	hci_reset();
}
